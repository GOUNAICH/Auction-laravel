...
class ProductController extends Controller
{
    ...
    function addProduct(Request $req) {
        ...
        $product->category = $req->input('category'); // Set the category
        if ($req->hasFile('file')) {
    ...
    public function Incremment($pid, Request $req) 
    {
        $userId = Auth::id();
        $product = Product::find($pid);
        if ($product->user_id === $userId) {
            return response()->json(['error' => 'You cannot auction your own product'], 403);
        }
        if ($req->has('price')) {
            $product->price = $req->input('price');
        }
    ...
    public function getProductsByCategory($category)
    {
        ...
        if ($products->isEmpty()) {
            // Retourner une réponse avec un message approprié
            return response()->json(['message' => 'Aucun produit trouvé dans la catégorie spécifiée.'], 404);
        }
